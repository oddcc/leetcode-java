        ...
        private StringBuilder convert(Integer integer) {
            ...
            for (int i = 0; i < range.length; i++) {
                if (integer >= range[i][0] && integer <= range[i][1]) {
                    ...
                    // 说明是1000
                    if (t == 3) {
                        sb.append(StringUtils.repeat(String.valueOf(roman[t][0]), integer / range[i][0]));
                        break;
                    }
                    // 说明是1～3
                    if (i % 2 == 0) {
                    ...
                    // 说明是4～9
                        ...
                        // > 5
                            ...
                            sb.append(roman[t][1]);
                            sb.append(StringUtils.repeat(String.valueOf(roman[t][0]), Math.max(0, (integer - (range[i][0] / 4) * 5) / (range[i][0] / 4))));
                        }
                        // == 5
                        else if (integer == (range[i][0] / 4) * 5) {
                            sb.append(roman[t][1]);
